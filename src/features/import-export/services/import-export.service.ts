...
function classifyImportFailure(
  ...
  if (lower.includes("stage_not_found") || lower.includes("service_not_found")) {
...
export async function importBeneficiaries(
  ...
  try {
    ...
    const [stagesResult, classesResult] = await Promise.all([
      supabase.from("stages").select("id, name_ar, name_en, service_id").eq("church_id", churchId).is("deleted_at", null),
      supabase.from("classes").select("id, name_ar, name_en").eq("church_id", churchId).is("deleted_at", null),
    ]);
    ...
    for (const row of rows) {
      ...
      // When a stage is provided, create the beneficiary and its current
      // assignment atomically through the sanctioned RPC (024); otherwise fall
      // back to a plain insert (no assignment is possible without a stage).
      const { error: insertError } = stageRef
        ? await supabase.rpc("create_beneficiary_with_assignment", {
            p_full_name_ar: beneficiaryFields.full_name_ar,
            p_service_id: stageRef.serviceId,
            p_stage_id: stageRef.id,
        ...
        : await supabase.from("beneficiaries").insert(beneficiaryFields);
  ...
  try {
    ...
    if (entity === "beneficiaries") {
      const { data, error } = await supabase
        .from("beneficiaries")
        .select("full_name_ar, full_name_en, mobile, date_of_birth, address, notes, status, created_at")
        .eq("church_id", churchId)
        .is("deleted_at", null)
