...
import type { Permission, PermissionCheckResult, Role, RbacError, NormalizedRbacState, UserPermissionsResult, UserRole } from "../types/rbac.types";
...
function toRbacError(code: keyof typeof RBAC_ERROR_MESSAGES, message?: string): RbacError {
  return {
    code,
...
async function loadCurrentUserRbac(churchId?: string): Promise<{ data: NormalizedRbacState | null; error: RbacError | null }> {
  try {
...
export async function getCurrentUserRoles(churchId?: string): Promise<{ data: Role[] | null; error: RbacError | null }> {
  const result = await loadCurrentUserRbac(churchId);
