    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, church_id, academic_year, status, confirmed_at, confirmed_by, created_at, " +
          "promotion_entries(beneficiary_id, from_stage_id, from_stage(service_id), undone_at), " +
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, church_id, stage_id, academic_year, status, notes, run_by, created_at, " +
          "stage(name_ar, name_en, service(name_ar, name_en)), " +
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, annual_promotion_id, church_id, beneficiary_id, from_stage_id, to_stage_id, " +
          ...
          "from_stage(name_ar, name_en), to_stage(name_ar, name_en)",
      )
      .eq("cycle_id", cycleId)
    ...
    const { data, error } = await supabase
      ...
      .select(
        "id, church_id, cycle_id, servant_id, status, " +
          ...
          "from_stage(name_ar, name_en, service(name_ar, name_en)), " +
          "to_stage(name_ar, name_en, service(name_ar, name_en))",
      )
      .eq("cycle_id", cycleId)
      .order("created_at", { ascending: true });
